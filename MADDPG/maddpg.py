...
class MADDPG():
    ...
    def get_actions(self, states):
        ...
        for agent, state in zip(self.agents, states):
            ...
            print("action_one_hot:" + str(action_one_hot))
    ...
    def compute_loss(self, batch_size, timestep):
        experiences = self.replay_buffer.sample(batch_size)
        states_batch, actions_batch, rewards_batch, next_states_batch, done_batch = experiences

        total_state_batch = [np.concatenate(states) for states in states_batch]
        action_batch = [np.concatenate(actions) for actions in actions_batch]
        ...
        for sample in range(batch_size):
            print("+++++++++++++++++++")
            print("states " + str(sample) + ": " + str(total_state_batch[sample]))
            print("actions " + str(sample) + ": " + str(action_batch[sample]))
            print("next_states" + str(sample) + ": " + str(next_state_batch[sample]))
            print("++++++++++++++++++")

        for agent in self.agents:
            curr_Q = agent.critic.forward(total_state_batch, action_batch)
    ...
    def update(self, batch_size, timestep):
        ...
        print("+++++++++++++++++++")
        print("total_state_batch" + str(total_state_batch))
        print("action_batch" + str(action_batch))
    ...
    def train(self, max_episodes, max_steps, batch_size):
        ...
        for episode in range(max_episodes):
            ...
            for steps in range(max_steps):
                ...
                #print("+++++++++++++++++++++++++++++++++++++++++++++")
                #print("step: " + str(steps) + " actions" + str(actions))
                new_states, rewards, dones, _ = self.env.step(actions)
                #print("step: " + str(steps) + " new_states" + str(new_states))
                #print("step: " + str(steps) + " rewards" + str(rewards))
                #print("+++++++++++++++++++++++++++++++++++++++++++++")
                ...
                if len(self.replay_buffer) > batch_size:
                    self.compute_loss(batch_size, timestep)
